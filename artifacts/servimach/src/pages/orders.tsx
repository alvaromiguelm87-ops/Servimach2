...
export default function Orders() {
  ...
  if (isLoading || !profile) {
    return <div className="p-8 text-center animate-pulse">A carregar encomendas...</div>;
  }

  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">As Suas Encomendas</h1>
        <p className="text-muted-foreground mt-1">
          {profile.role === 'client' ? 'Histórico de pedidos efetuados.' : 'Histórico de serviços.'}
        </p>
      </div>
        ...
        {!orders || orders.length === 0 ? (
          <Card>
            <CardContent className="p-12 text-center text-muted-foreground flex flex-col items-center justify-center">
              ...
              <p className="text-lg">Não tem encomendas registadas.</p>
              {profile.role === 'client' && (
          ...
          orders.map(order => (
            <OrderListItem key={order.id} order={order} role={profile.role as any} />
          ))
        )}
      </div>
    </div>
...
function OrderListItem({ order, role }: { order: Order, role: 'client' | 'professional' | 'admin' }) {
  return (
        ...
        <div className="flex flex-col sm:flex-row gap-6 justify-between">
          <div className="space-y-3 flex-1">
            <div className="flex items-center gap-3">
              <span className="font-semibold text-lg">
                {role === 'client' || role === 'admin' ? order.professionalName : order.clientName}
              </span>
          ...
          <div className="flex sm:flex-col items-center sm:items-end justify-between gap-4 border-t sm:border-t-0 sm:border-l border-border pt-4 sm:pt-0 sm:pl-6">
            <div className="text-right">
              <div className="text-xl font-bold text-primary">{order.price.toFixed(2)}€</div>
              {role === 'professional' && (
                <div className="text-xs text-muted-foreground mt-1">
                  Ganhos líquidos: <span className="font-medium text-foreground">{order.professionalEarnings.toFixed(2)}€</span>
                </div>
              )}
            </div>
            <Button asChild>
